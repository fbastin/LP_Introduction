...
Considérons par exemple le centre analytique associé à un polytope borné $\Omega$ dans $E^m$, représenté par $n$ ($> m$) inégalités linéaires:
\[
\Omega = \lbrace \by \in E^m \,:\, \bc^T - \by^T \bA \geq \bzero \rbrace,
\]
où $\bA \in E^{m \times n}$ et $c \in E^n$ sont données, et $A$ est de rang $m$.
...
\[
\mathring{\Omega} = \lbrace \by \in E^m \,:\, \bc^T - \by^T\bA > 0 \rbrace.
\]
...
\]
Le centre analytique est alors défini par les conditions
\begin{align*}
\bx \circ \bs &= \bone \\
\bA\bx &= \bzero \\
\bA^T\by + \bs & = \bc.
...
\[
\Omega = \lbrace \by \in E^m \,:\, \bc^T-\by^T\bA \geq 0,\, \bB\by = \bb \rbrace
\]
...
\begin{description}
	...
	A^Ty_0 + s_0 &= c^T
	\end{align*}
	et $y_0, s_0 \geq 0$, $x_0 > 0$. Poser $k = 0$, et choisir $\epsilon$.
	\item[Etape 2]
...
Il est facile d'exprimer le gradient et le hessien de ce problème:
\begin{align*}
...
$$
et multiplions de part et d'autre par $AX^2$. Nous obtenons
$$
...
L'étape prédictive consiste à calcul le pas de Newton classique, en résolvant
$$\begin{pmatrix}
	0 & A^T & I  \\
	A & 0 & 0\\
	I & 0 & S^{-1}X
